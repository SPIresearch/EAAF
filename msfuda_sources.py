...
def model_forward(mddn_F, mddn_C1, mddn_C2, mddn_E1, mddn_E2, X, y):
    ...
    loss2 = nn.CrossEntropyLoss()(output, y)

    loss = loss2
    return evidence, loss


def evi_model_forward(args, mddn_F, mddn_C1, mddn_C2, mddn_E1, mddn_E2, X, y):
    fea = mddn_F(X)
    output = mddn_C2(mddn_C1(fea))
    ...
    loss1 = loss_func.ce_loss(y, alpha, args.class_num)
...
def cal_acc(loader, mddn_F, mddn_C1, mddn_C2, flag=False):
    ...
    mean_ent = torch.mean(loss_func.Entropy(all_output)).cpu().data.item()
...
def train_source_step1(args):
    ...
    mddn_C1 = network.feat_bottleneck(
        type=args.bn, feature_dim=mddn_F.in_features, bottleneck_dim=args.bottleneck
    ).cuda()
    mddn_C2 = network.feat_classifier(
        type=args.layer, class_num=args.class_num, bottleneck_dim=args.bottleneck
    ).cuda()
    mddn_E1 = network.feat_bottleneck(
        type=args.bn, feature_dim=mddn_F.in_features, bottleneck_dim=args.bottleneck
    ).cuda()
    mddn_E2 = network.evidence_classifier(
    ...
    while iter_num < max_iter:
        ...
        _, classifier_loss = model_forward(
            mddn_F, mddn_C1, mddn_C2, mddn_E1, mddn_E2, inputs_source, labels_source
        )
        optimizer.zero_grad()
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            mddn_C2.eval()
            acc_s_te, _ = cal_acc(
                dset_loaders["source_te"], mddn_F, mddn_C1, mddn_C2, False
            )
...
def test_target(args):
    ...
    mddn_C2 = network.evidence_classifier(
        type=args.layer, class_num=args.class_num, bottleneck_dim=args.bottleneck
    ).cuda()
...
def cal_acc_test(loader, mddn_F, mddn_C1, mddn_C2, flag, args):
    ...
    scipy.io.savemat(
        f"hsn_{str(args.s)}_{str(args.t)}_tsne.mat",
        {
...
def train_source_step2(args, mddn_F, mddn_C1, mddn_C2, mddn_E1, mddn_E2):
    ...
    while iter_num < max_iter:
        ...
        _, classifier_loss = evi_model_forward(
            args,
            mddn_F,
            mddn_C1,
            mddn_C2,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--net", type=str, default="resnet50", help="vgg16, resnet50, resnet101"
    )
    ...
    for k in range(len(names)):
        ...
        train_source_step2(args, mddn_F, mddn_C1, mddn_C2, mddn_E1, mddn_E2)
        args.out_file = open(
