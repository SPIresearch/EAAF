...
def train_source1(args):
    ...
    acc_init = 0
    max_iter = 40 * len(dset_loaders["source_tr"])
    ...
    torch.save(best_netF, osp.join(args.output_dir_src_source, "source_F.pt"))
    torch.save(best_netB, osp.join(args.output_dir_src_source, "source_B.pt"))
    torch.save(best_netC, osp.join(args.output_dir_src_source, "source_C.pt"))
    torch.save(best_netE, osp.join(args.output_dir_src_source, "source_E.pt"))
    torch.save(best_netEC, osp.join(args.output_dir_src_source, "source_EC.pt"))
...
def train_source2(args):
    ...
    acc_init = 0
    max_iter = 10 * len(dset_loaders["source_tr"])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='ckps/source_mul_evi1')
    parser.add_argument('--trte', type=str, default='full', choices=['full', 'val'])
    parser.add_argument('--output_src', type=str, default='ckps/source_mul1')
    ...
    for k in range(len(names)):
        ...
        args.output_dir_src_source = osp.join(args.output_src, args.dset, names[args.s][0].upper())
